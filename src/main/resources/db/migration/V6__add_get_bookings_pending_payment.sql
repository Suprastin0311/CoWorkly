/*
 ...
 Source Server         : My Test Conncetion
...
-- ----------------------------
BEGIN;
INSERT INTO "bookings" ("id", "user_id", "workspace_id", "start_time", "end_time", "participants_count", "status_id", "price", "created_at") VALUES (4, 2, 8, '2026-09-02 10:00:00', '2026-09-02 13:25:00', 3, 1, 12800.00, '2026-08-30 17:53:05.171555'), (5, 2, 16, '2026-09-02 10:00:00', '2026-09-02 11:15:00', 1, 1, 670.00, '2026-08-30 17:53:31.420531'), (6, 2, 1, '2026-09-02 10:00:00', '2026-09-02 15:40:00', 1, 1, 4500.00, '2026-08-30 17:53:53.244787'), (3, 2, 4, '2026-09-02 10:00:00', '2026-09-02 12:30:00', 1, 5, 1800.00, '2026-08-30 17:52:42.2507'), (7, 3, 4, '2026-09-08 10:00:00', '2026-09-08 13:00:00', 1, 1, 120000.00, '2026-09-04 21:51:07.28278'), (9, 3, 4, '2026-09-04 22:32:00', '2026-09-04 22:35:00', 1, 1, 6.00, '2026-09-04 22:31:34.743715'), (10, 3, 4, '2026-09-08 18:30:00', '2026-09-08 18:35:00', 1, 1, 10.00, '2026-09-08 17:37:47.07946'), (8, 3, 5, '2026-09-08 10:00:00', '2026-09-08 13:00:00', 1, 2, 630.00, '2026-09-04 21:53:00.004768');
COMMIT;
...
CREATE OR REPLACE FUNCTION "confirm_booking"("booking_id_param" int8)
  RETURNS "pg_catalog"."bool" AS $BODY$BEGIN
	UPDATE bookings
	SET status_id = (SELECT id FROM booking_statuses WHERE name = 'CONFIRMED')
...
-- ----------------------------
-- Function structure for get_bookings_pending_payment
-- ----------------------------
DROP FUNCTION IF EXISTS "get_bookings_pending_payment"("user_id_param" int8) CASCADE;
CREATE OR REPLACE FUNCTION "get_bookings_pending_payment"("user_id_param" int8=NULL::bigint)
  RETURNS TABLE("booking_id" int8, "workspace_id" int8, "wtype_id" int8, "user_id" int8, "user_email" text, "user_full_name" text, "workspace_type" varchar, "workspace_name" text, "start_time" timestamp, "end_time" timestamp, "participants_count" int4, "status" varchar, "price" numeric, "created_at" timestamp) AS $BODY$BEGIN
	RETURN QUERY
	SELECT * FROM get_bookings(
		"user_id_param", 
		NULL, 
		(SELECT s.id FROM booking_statuses s WHERE s."name" = 'PENDING_PAYMENT'),
		NULL, 
		NULL);
END$BODY$
  LANGUAGE plpgsql VOLATILE
...
-- ----------------------------
DROP FUNCTION IF EXISTS "get_user_by_email"("email_param" text) CASCADE;
...
-- Function structure for insert_booking
-- ----------------------------
DROP FUNCTION IF EXISTS "insert_booking"("user_id_param" int8, "workspace_id_param" int8, "start_time_param" timestamp, "end_time_param" timestamp, "participants_count" int4) CASCADE;
...
-- Function structure for insert_booking
-- ----------------------------
DROP FUNCTION IF EXISTS "insert_booking"("user_id_param" int8, "workspace_id_param" int8, "start_time_param" timestamp, "end_time_param" timestamp, "participants_count_param" int4, "price_param" int4) CASCADE;
...
SELECT setval('"bookings_id_seq"', 10, true);
...
SELECT setval('"workspaces_id_seq"', 18, true);
