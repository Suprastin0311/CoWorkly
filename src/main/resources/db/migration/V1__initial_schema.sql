/*
  PostgreSQL 18 restore script
  Generated from the supplied Navicat export.

  IMPORTANT: this script completely recreates schema public in the currently
  selected database. Run it while connected to the empty target database.
*/
...
-- Sequences must exist before tables whose DEFAULT expressions reference them.
CREATE SEQUENCE booking_statuses_id_seq AS bigint;
CREATE SEQUENCE bookings_id_seq         AS bigint;
CREATE SEQUENCE day_types_id_seq        AS bigint;
CREATE SEQUENCE tariffs_id_seq          AS bigint;
CREATE SEQUENCE user_roles_id_seq       AS bigint;
CREATE SEQUENCE users_id_seq            AS bigint;
CREATE SEQUENCE users_role_seq          AS bigint;
CREATE SEQUENCE workspace_types_id_seq  AS bigint;
CREATE SEQUENCE workspaces_id_seq       AS bigint;
...
-- Records of tariffs
-- ----------------------------
INSERT INTO "tariffs" ("id", "workspace_type", "day_type", "multiplier") VALUES (1, 8, 1, 5.00), (2, 8, 2, 7.50), (3, 9, 1, 10.00), (4, 9, 2, 15.00);
...
-- Records of users
-- ----------------------------
INSERT INTO "users" ("id", "email", "password_hash", "full_name", "role", "is_blocked", "created_at") VALUES (1, 'dd.nikolaenko@gmail.com', '$2a$12$yu7io7as/LNscjJTdBlet.HjCR6F9TPxKetvjfn9DNGftYI5fBrN6', 'Николаенко Дмитрий Денисович', 1, 'f', '2026-08-02 18:15:17.158'), (2, '1@user.test', '$2a$12$dfiDQYUuiJpSX0oXrq48IOw2I.1.fggf4gll0bLi1RI8k7IUa4gKm', 'Первый Тестовый Пользователь', 2, 't', '2026-08-02 18:17:42.808'), (3, '2@user.test', '$2a$12$sZ8GiOtVbePScH7xsCW91eD/FumDoWuhsPfv9P/A.cpJqm6BY9gnq', 'Второй Тестовый Пользователь', 2, 'f', '2026-08-27 00:02:15.809'), (4, '3@user.test', '$2a$12$Nl3dEcbTXoujpcmSkx1xO.i.APhhg6AobQgVFqObPI55PgpSQTgoW', 'Третий Тестовый Пользователь', 2, 'f', '2026-08-27 00:04:11.101'), (5, '4@user.test', '$2a$12$WXAjVTlOEvLgwxQxyVMjK.c3SJflL6MGObHs/BPE7L5iyFTveGvt6', '������� ���⮢� ���짮��⥫�', 2, 'f', '2026-08-27 00:26:05.09');
...
-- ----------------------------
DROP FUNCTION IF EXISTS "bookings_by_user_id"("user_id_param" int8);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "confirm_booking"("booking_id_param" int8);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "delete_workspace"("id_param" int8);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "get_bookings"("user_id_param" int8, "workspace_id_param" int8, "status_id_param" int8, "start_time_param" timestamp, "end_time_param" timestamp);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "get_bookings_by_workspace_id"("w_id_param" int8);
...
-- ----------------------------
-- Function structure for get_user_by_email
-- ----------------------------
DROP FUNCTION IF EXISTS "get_user_by_email"("email_param" text);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "get_users"("id_param" int8, "email_param" text, "full_name_param" text, "role_id_param" int8, "is_blocked_param" bool, "min_created_at_param" timestamp, "max_created_at_param" timestamp);
CREATE OR REPLACE FUNCTION "get_users"("id_param" int8=NULL::bigint, "email_param" text=NULL::text, "full_name_param" text=NULL::text, "role_id_param" int8=NULL::bigint, "is_blocked_param" bool=NULL::boolean, "min_created_at_param" timestamp=NULL::timestamp without time zone, "max_created_at_param" timestamp=NULL::timestamp without time zone)
  RETURNS TABLE("id" int8, "email" text, "password_hash" text, "full_name" text, "role" varchar, "role_id" int8, "is_blocked" bool, "created_at" timestamp) AS $BODY$
	DECLARE query_text TEXT := $sql$
		SELECT 
				u.id, 
				u.email,
...
-- ----------------------------
DROP FUNCTION IF EXISTS "get_workspace_by_id"("id_param" int8);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "get_workspaces"("id_param" int8, "capacity_param" int4, "min_hourly_rate_param" numeric, "max_hourly_rate_param" numeric, "name_param" text, "is_active_param" bool, "type_id_param" int8);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "get_workspaces_available_for_booking"("start_time_param" timestamp, "end_time_param" timestamp, "workspace_type_id_param" int8, "participants_count_param" int4);
  ...
  RETURNS TABLE("id" int8, "type_id" int8, "type_name" varchar, "type_name_rus" varchar, "name" text, "capacity" int4, "min_participants_count" int4, "max_participants_count" int4, "hourly_rate" numeric, "price" numeric) AS $BODY$BEGIN
	...
	RETURN query (
		SELECT
			...
			-- Количество часов --
			(CEIL(EXTRACT(EPOCH from (end_time_param - start_time_param)) / 3600.0)
				-- Множитель тарифа --
...
-- ----------------------------
DROP FUNCTION IF EXISTS "insert_booking"("user_id_param" int8, "workspace_id_param" int8, "start_time_param" timestamp, "end_time_param" timestamp, "participants_count" int4);
CREATE OR REPLACE FUNCTION "insert_booking"("user_id_param" int8, "workspace_id_param" int8, "start_time_param" timestamp, "end_time_param" timestamp, "participants_count" int4)
	...
	BEGIN
		...
		VALUES 
			(
				...
				-- Множитель тарифа --
				* (SELECT multiplier FROM tariffs t
					WHERE t.workspace_type = (SELECT type FROM workspaces w WHERE w."id" = workspace_id_param) 
					AND day_type = CASE 
									WHEN DATE_PART('dow', start_time_param) IN (1, 2, 3, 4, 5) THEN 1 -- Будни --
									WHEN DATE_PART('dow', start_time_param) IN (6, 0) THEN 2 -- Выходные --
									END)
				-- Цена за час --
				* (SELECT w.hourly_rate FROM workspaces w WHERE w."id" = workspace_id_param)),
				NOW()
			)
...
-- Function structure for insert_booking
-- ----------------------------
DROP FUNCTION IF EXISTS "insert_booking"("user_id_param" int8, "workspace_id_param" int8, "start_time_param" timestamp, "end_time_param" timestamp, "participants_count_param" int4, "price_param" int4);
CREATE OR REPLACE FUNCTION "insert_booking"("user_id_param" int8, "workspace_id_param" int8, "start_time_param" timestamp, "end_time_param" timestamp, "participants_count_param" int4, "price_param" int4)
  RETURNS "pg_catalog"."int8" AS $BODY$
	DECLARE
		new_booking_id int8;
	BEGIN
...
-- ----------------------------
DROP FUNCTION IF EXISTS "insert_user"("email_param" text, "password_hash_param" text, "full_name_param" text, "role_param" int8, "is_blocked_param" bool, "created_at_param" timestamp);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "insert_workspace"("type_id_param" int8, "name_param" text, "capacity_param" int4, "hourly_rate_param" numeric, "is_active_param" bool);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "set_booking_cancelled"("id_param" int8);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "toggle_user_active_status"("id_param" int8);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "toggle_workspace_active_status"("id_param" int8);
...
-- ----------------------------
DROP FUNCTION IF EXISTS "update_workspace"("id_param" int8, "type_id_param" int8, "name_param" text, "capacity_param" int4, "hourly_rate_param" numeric, "is_active_param" bool);
...
-- Bind sequences to their columns and advance them past imported identifiers.
ALTER SEQUENCE booking_statuses_id_seq OWNED BY booking_statuses.id;
...
ALTER SEQUENCE users_role_seq          OWNED BY users.role;
ALTER SEQUENCE workspace_types_id_seq  OWNED BY workspace_types.id;
ALTER SEQUENCE workspaces_id_seq       OWNED BY workspaces.id;

SELECT setval('booking_statuses_id_seq', COALESCE((SELECT max(id) FROM booking_statuses), 1), EXISTS (SELECT 1 FROM booking_statuses));
SELECT setval('bookings_id_seq',         COALESCE((SELECT max(id) FROM bookings), 1),         EXISTS (SELECT 1 FROM bookings));
SELECT setval('day_types_id_seq',        COALESCE((SELECT max(id) FROM day_types), 1),        EXISTS (SELECT 1 FROM day_types));
SELECT setval('tariffs_id_seq',          COALESCE((SELECT max(id) FROM tariffs), 1),          EXISTS (SELECT 1 FROM tariffs));
SELECT setval('user_roles_id_seq',       COALESCE((SELECT max(id) FROM user_roles), 1),       EXISTS (SELECT 1 FROM user_roles));
SELECT setval('users_id_seq',            COALESCE((SELECT max(id) FROM users), 1),            EXISTS (SELECT 1 FROM users));
SELECT setval('users_role_seq',          COALESCE((SELECT max(role) FROM users), 1),          EXISTS (SELECT 1 FROM users));
SELECT setval('workspace_types_id_seq',  COALESCE((SELECT max(id) FROM workspace_types), 1),  EXISTS (SELECT 1 FROM workspace_types));
SELECT setval('workspaces_id_seq',       COALESCE((SELECT max(id) FROM workspaces), 1),       EXISTS (SELECT 1 FROM workspaces));
